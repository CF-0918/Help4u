  ...
  class _ServiceFeedbackState extends State<ServiceFeedback> {
    ...
    Future<void> _loadBookingDetails() async {
      ...
      // 1. Fetch the booking row by booking_id
      ...
      debugPrint("Booking fetched: $booking");

      if (booking == null) {
        debugPrint("No booking found for booking_id ${widget.bookingId}");
        return;
      }

      // 2. Fetch the service_type row using servicetypeid from booking
      final serviceType = await supabase
          .from('service_type')
          .select('name')
          .eq('id', booking['servicetypeid'])
          .maybeSingle();

      debugPrint("ServiceType fetched: $serviceType");

      // 3. Update UI state with booking + service_type data
      setState(() {
        _serviceName = serviceType?['name'] ?? "Unknown Service";
        _vehiclePlate = booking['vehicleplateno'] ?? "--";
        _completedDate = booking['bookingdate'] ?? "--";
        _completedTime = booking['bookingtime'] ?? "--";
      });
    }
